<template>
  <el-container class="MessageLists">
    ...
    <el-main><!-- 編輯彈窗頁面 catalogueDialog"-->
      <div class="ty-header-list2">
        <el-col :span="2"><i class="el-icon-document"></i> 待处理事项</el-col>
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              size="mini"
              type="primary"
              @click="handlecompile(scope.$index, scope.row.serviceOrderId,scope.row.status)">查看订单</el-button>
          </template>
        </el-table-column>
        <!--删除-->
        <el-table-column label="" prop="status">
          <template slot-scope="scope">
            <el-button
              ...
              @click="editformdelet(scope.$index, scope.row.id,scope.row.status)">{{scope.row.status}}</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!--TODO 页数标签-->
