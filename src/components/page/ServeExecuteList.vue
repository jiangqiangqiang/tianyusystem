...
&lt;!&ndash;
              ...
              <!--<el-button icon="el-icon-search" type="primary"></el-button>-->
                ...
                <el-date-picker type="date" placeholder="选择日期" v-model="start" style="width: 100%;" format="yyyy-MM-dd" value-format="yyyy-MM-dd"></el-date-picker>



              <el-date-picker type="date" placeholder="选择日期" v-model="end" style="width: 100%;" format="yyyy-MM-dd" value-format="yyyy-MM-dd"></el-date-picker>
              ...
              <span @click="getExecuteList">筛选</span>
						</div>
		...
		<el-main>
			...
			<el-table :data="tableData" style="width: 100%">
				...
				<el-table-column prop="visitAddress" label="就诊地点"></el-table-column>
				<el-table-column prop="visitType" label="就诊类型"></el-table-column>
				<el-table-column prop="serviceName" label="服务人员"></el-table-column>
				<el-table-column prop="serviceMobile" label="电话"></el-table-column>
				<el-table-column prop="productName" label="服务项"></el-table-column>
				...
				<el-table-column label="操作">
					<template slot-scope="scope">
						<el-button size="mini" type="primary" @click="updateReservations(scope.$index)">修改预约</el-button>
					</template>
				</el-table-column>
				<el-table-column label="">
					<template slot-scope="scope">
						<el-button size="mini" type="primary" @click="handlecompile(scope.$index, scope.row.serviceOrderId,scope.row.status)">查看订单</el-button>
					</template>
				</el-table-column>
			...
			<!--页数标签-->
			<div class="pages">
				<el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page.sync="currentPage" :page-sizes="[10, 20, 30, 40]" :page-size="10" background layout="sizes,prev, pager, next" :total="pageNum">
				</el-pagination>
			</div>
		</el-main>
		<!--修改预约-->
			...
			<el-dialog :modal="false" title="修改预约" :visible.sync="centerDialogVisible" width="25%" center>
				<el-form :model="updataOrder">
					<el-form-item label="就诊时间" :label-width="formLabelWidth">
						<el-date-picker v-model="updataOrder.time" type="datetime" placeholder="选择日期时间" format="yyyy-MM-dd HH:mm" value-format="yyyy-MM-dd HH:mm">
						</el-date-picker>
					</el-form-item>
	...
	export default {
		...
		methods: {
			...
			handlecompile(index,id,status){ //表单编辑按钮
				console.log(id)
        this.$router.push({name:'ServiceOrderDetails',params:{task_id :id,token_TO: localStorage.getItem('currentUser_token'),status_TO:status}})
      },
			getExecuteList() {
				var self = this;
				var token = localStorage.getItem('currentUser_token');
          ...
          query:{
			...
			getDockerList() {
				...
				axios.post(self.baseURl + '/servicepeople/list', fd, config).then(res => {
					if(res.data.code == 0) {
